    ...
    <AnimatePresence>
      {visible && (
        <motion.nav
          ...
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 hidden sm:flex items-center gap-1 bg-surface/95 backdrop-blur-md border border-outline-variant/40 rounded-full p-1.5 lux-shadow"
        >
          {items.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center rounded-full transition-all duration-300 ${
                  active
                    ? 'gap-2 bg-on-surface text-surface px-4 py-2.5'
                    : 'flex-col gap-0.5 px-3 py-2 text-on-surface-variant hover:text-primary'
                }`}
              >
                <span className={`material-symbols-outlined ${active ? 'text-[20px]' : 'text-[19px]'}`}>
                  {item.icon}
                </span>
                <span
                  className={`font-label-caps tracking-wide whitespace-nowrap ${
                    active ? 'text-[12px]' : 'text-[10px]'
                  }`}
                >
                  {item.label}
                </span>
